 ...
 */
export default class PerformanceRender {
	/**
	...
	constructor(Performance, Ticker) {
		...
		Ticker.on("afterTick", this.update);
	}
	update() {
		...
		if (enabled) {
			...
			// background
			ctx.beginPath();
			RenderMethods.roundedRect(width, height, new Vec2(width/2, height/2), 5, ctx);
			...
			if (history.fps.length > 10) { // fps graph
				...
				let bounds = {
					min: new Vec2(10, 18),
					max: new Vec2(width - 10, height - 4),
				};
			...
			RenderMethods.roundedRect(6, 6, new Vec2(15, 13), 2, ctx);
			ctx.fill();
